 //                                       CENTRAL TELEFONICA TELEFONO DE LINEA
...
int main(int argc, char* argv[])
{
	...
	if(argc!=2) 
	{
		logger("Error","CentralLinea", "Debo usar:./cenL < archivo de configuracion >", 1);
		return -1;
	}
...
//system("clear");//Limpia la Pantalla
	...
	if( cargarConfigCentral( 	argv[1],
								...
								centralID ) == -1 )
	{
		logger("Error","CentralLinea", "No se pudo cargar el archivo de Configuracion ", 1);
		return -1;
	}
	...
	{
	    logger("Informacion","CentralLinea", "Primer Central. No se conectara a ninguna otra", 0);
	    estoyConectado=1;
	}	
	else 
	{
	...
	printf("***********************************************************************\n");
	printf("* NO HAY TELEFONOS CONOCIDOS                                          *\n");
	printf("***********************************************************************\n");
		...
		for(i=0;i<=fdmax;i++)
		{
			...
			{ 
				...
				if(i==listener)	
					...
					{
						logger("Error","CentralLinea", "Fallo AtenderConexion", 0);
						return -1;
					}
				if(i!=listener)
				{
					...
					if((numbytes=recv(i,buffer,sizeof(cabecera),0))<=0)
					{
						...
						{
							printf("\tTelefono quitado de nuestra red de clientes\n");
							if( colaFte != NULL )
							{		
								...
								logger("Informacion","Central", "Nuevo telefono conectado a nuestra red de clientes", 1);
								mostrarVector( vcelulares, topeVector);
								if( send(aux.socket,"CONZ",5,0) < 0)
								{
									logger("Error","CentralLinea", "Fallo el send de CONZ", 0);
									return -1;
								}
							}		
						}
						...
						{	
							//conexion cerrada
							logger("Informacion","CentralLinea", "Conexion Cerrada", 1);
							close(i);
							FD_CLR(i,&master);
						}
						else	
						{ 	//Error!
							logger("Error","CentralLinea", "Fallo recepcion de datos", 0);
							close(i);
							FD_CLR(i,&master);
						}
					}
					...
					{ 	//No hay errores y hay datos
						if(atenderCliente(	buffer,
											...
											&fdmax	)==-1)
						{			
							logger("Error","CentralLinea", "Fallo atenderCliente", 0);
							return -1;
						}
					}	
